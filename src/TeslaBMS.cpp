...
void loop()
{
    ...
    if (millis() > (lastUpdate + 1000))
    {
        ...
        if (autoBalance)
        {
            if (++balanceTime > BALANCE_INTERVAL)
            {
                balanceTime = 0;

                // kick off balancing for 10 seconds less than the interval - to give time for the cell vlotages to stabilise
                if (!bms.balanceCells(gwiz.getLowestCellVoltage(), BALANCE_INTERVAL - 10, 1))
                {
                    autoBalance = false;
                }
            }
